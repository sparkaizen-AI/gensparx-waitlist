...
export default function Home() {
  ...
  const testConnection = async () => {
    ...
    try {
      const response = await fetch('https://ntlmkgbxhnhkzfnfwsnx.supabase.co/rest/v1/waitlist?limit=1', {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        setTestResult(`✅ Connection OK! Found ${data.length} entries`);
      } else {
        setTestResult(`❌ Connection failed: ${response.status}`);
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("🔗 API URL:", 'https://ntlmkgbxhnhkzfnfwsnx.supabase.co/rest/v1/waitlist');
      
      // Try with different approach - use POST with explicit content type
      ...
      setIsSuccess(true);
      setMessage("Successfully added to waitlist! Check your Supabase dashboard.");
      ...
      <main className="relative z-10 w-full max-w-2xl px-6 flex flex-col items-center text-center">
        ...
        <div className="w-full max-w-[340px] mb-4">
          <button
            ...
            className="w-full h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 hover:border-blue-500/50 text-white text-sm font-medium transition-all duration-300"
          >
        ...
        {/* Test Result */}
        {testResult && (
          <div className="w-full max-w-[340px] mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <p className="text-xs text-blue-300 font-mono break-all">{testResult}</p>
          </div>
        )}
