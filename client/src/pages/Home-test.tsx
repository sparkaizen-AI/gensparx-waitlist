...
export default function Home() {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      const test1 = await fetch(`${supabaseUrl}/rest/v1/waitlist?limit=1`, {
        method: 'GET',
        headers: {
          'apikey': supabaseKey,
      ...
      const test2 = await fetch(`${supabaseUrl}/rest/v1/`, {
        method: 'POST',
        headers: {
          'apikey': supabaseKey,
          'Authorization': `Bearer ${supabaseKey}`,
        ...
        body: JSON.stringify({
          ...
          email: "test@gensparx.com"
        })
      });
      
      setTestResults([
        { test: "GET Request", status: test1.status, ok: test1.ok },
        { test: "POST Request", status: test2.status, ok: test2.ok }
      ]);
      
      setDebugInfo(`✅ Connection test completed. Check console for details.`);
      ...
      <main className="relative z-10 w-full max-w-2xl px-6 flex flex-col items-center text-center">
        ...
        {testResults.length > 0 && (
          <div className="w-full max-w-[340px] mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <h4 className="text-sm font-medium text-blue-300 mb-2">Connection Test Results:</h4>
